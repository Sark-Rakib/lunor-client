...
const HalfSleeve = () => {
  ...
  useEffect(() => {
    const fetchSweetProducts = async () => {
      try {
        const res = await axiosSecure.get("/products?category=halfsleeve");
  ...
  return (
    <section className="py-10">
      <title>Lunor | Half Sleeve</title>
      <div className="px-4 sm:px-7 md:px-6">
        {/* Title */}
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold text-black">
            All <span className="text-gray-400">Half Sleeve</span>
          </h2>
          <p className="text-gray-600 mt-1">Half Sleeve Shirt Collection</p>
        </div>
        ...
        {sweets.length === 0 && (
          <p className="text-center text-gray-500 mt-10">
            No Half Sleeve Shirt items found
          </p>
        )}
      </div>
    </section>
